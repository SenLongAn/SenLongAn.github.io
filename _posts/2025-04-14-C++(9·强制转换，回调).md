...
* 时机：在条件语句、初始化、赋值、函数实参，函数返回值……情况会自动隐式转换，使其类型匹配
* 支持的转换操作：
...
编译时转换，不会进行 运行时类型检查(转换失败不会返回nullptr), 可能造成运行时崩溃
...
* 算数类型之间的相互转换
  ...
  * 高精度-》低精度，仅存低精度范围
  * 低精度-》高精度，数值不变
  * 整数之间，bool，可以被整数赋值，false -> 0, true -> 1，char，可以被整数赋值, ASCII <-> 字符
  * 无符号《-》有符号
* 任何类型指针都可以赋值给void指针，void指针转换成目标类型的指针需要显示转换
* 继承体系中基类和派生类之间指针或引用的转换，派生到基类上行安全，下行不安全，要转换的指针指向的对象的实际类型与转换以后的对象类型一定要相同，才能转换成功
...
运行时转换，会运行时类型检查，因此可以针对多种情景做出应对方案，比static_cast对类层次转换更安全

继承体系中基类和派生类之间指针或引用的转换，要求父类中至少有一个虚函数（因为此转换机制依赖虚函数表），要转换的指针指向的对象的实际类型与转换以后的对象类型一定要相同，才能转换成功，转换失败返回空
...
* lambda/匿名表达式：
  ...
  * [capture list]:
    ...
    * 对于全局/文件/命名空间作用域/static修饰的可以直接访问，无需通过捕获列表捕获，但对于局部非静态/类非静态成员，需要捕获才能使用
    * []：默认不捕获任何变量；
  ...
  * 作用：
    ...
    * 定义在类内、函数内（作为临时工具函数）、参数列表中（作为被调用函数），非常灵活
    * 匿名防止名称冲突
* std::bind 函数适配器
  ...
  * 作用：
    * 减少参数数量，调整参数顺序，以便适配所需的参数形式（比如STL谓词）
