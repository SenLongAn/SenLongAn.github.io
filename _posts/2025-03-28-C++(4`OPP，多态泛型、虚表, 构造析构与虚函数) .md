...
非基类指针/引用 / 非虚函数，触发静态绑定

当通过基类的指针/引用调用一个虚函数时，触发动态绑定

**静态绑定、动态绑定：**

静态绑定：构建时，根据指针/引用类型，确定函数地址，在运行时直接通过函数地址调用，它在运行时效率更高（直接调用），但灵活性较差（构造函数比较特殊，虽然是静态绑定，但会按照构造顺序调用构造函数）

动态绑定：运行时，根据 指针/引用 指向/绑定 的对象类型，从虚表查找函数地址并调用，它在运行时效率更低（查表），但灵活性较高（允许更改指针指向，调用指向对象的函数）
...
* vtbl是一个函数指针数组，每个函数指针指向该类的一个虚函数
* 每个包含虚函数的类都有且仅有一个虚表（vtbl）
* vtbl是在编译阶段生成的
* 内存开销不大，因为都已指针形式存储
...
* 通过*__vptr指针来访问vtbl
* 每个包含虚函数的类对象，都含有一个vptr（相当于类成员）
* 存放在对象内存中，占用sizeof
* vptr是在编译阶段存放在类类型布局，在运行时随着类对象创建，在调用构造函数时进行初始化（vptr指向vtbl）
...
* 由于构造函数顺序，先调用A的构造，创建A_vptr,并让A_vptr指向A_Vtbl，分配A内存块，再调用B的构造，创建了B_vptr,并让B_vptr指向B_Vtbl，分配B内存块
* p指向bObject首地址
* p找到bObject的B_vptr，找到对应的虚函数地址调用
* 不会查表，直接调用A的函数
...
构造函数不能为虚函数：这个操作触发了动态绑定，需要查表来调用构造，但vptr还没有初始化，因此无法调用，并且c++语法规定，构造不能为虚函数，会出现编译错误
