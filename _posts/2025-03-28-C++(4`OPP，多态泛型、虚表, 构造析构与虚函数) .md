...
* 封装，是将数据和方法封装在一个类中，对外部隐藏内部实现细节，仅暴露必要的接口，提高安全性
* 继承，允许一个类继承另一个类，并可以扩展或重写父类的功能（新增，修改），提高复用和易于扩展
* 多态，是通过统一的接口操作不同类型的对象，从而产生不同的行为，提高复用性
...
* 当派生类修改继承的行为时，尽量用虚函数重写，因为它支持多态性，比普通的隐藏方式更灵活
* 当继承中的基类并非实体的含义，仅作为接口时，使用纯虚函数
