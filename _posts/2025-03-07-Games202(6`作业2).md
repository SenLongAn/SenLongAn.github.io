...
* L项系数，遍历cubemap环境贴图每个像素，遍历每个basic项，像素的颜色值*basic（角度）\*w权重，即蒙特卡洛采样一次的结果，角度使用像素方向向量，w使用像素在单位球体的面积CalcArea（）函数获得
* 漫反射无阴影LT项（cos项）系数：遍历每个顶点，shFunc（）函数负责获取顶点法向半球任意角度cos项值，ProjectFunction（）函数计算投影的系数数组，存储在总的LT系数数组中
* 漫反射无阴影LT项（Vcos项）系数：shFunc中多了个V项的计算，反射光线和场景求交
* 多bounce间接光照系数：函数接收某顶点数据…… ，随机生成采样方向，如果射线击中场景，则递归计算交点系数，将自身直接光照系数和递归获得的间接光系数相加，返回条件为超过最大bounce次数
* 光源旋转：利用作业提供的公式
...
#### 预计算 diffuse unshadowed LT coeffs
...
#### 预计算 diffuse shadowed LT coeffs
