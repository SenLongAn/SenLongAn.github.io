...
* 常量对象：
  * const关键字修饰变量时：定义一个常量，限定它不能被修改，是只读的，否则将发生编译错误，防止程序员意外修改
...
* 表达式
  * 操作数和操作符的组合，用于进行计算和求值，表达式的结果是一个值
  * 操作数：表达式中的数据，可以是常量、变量、函数调用的返回值，或者是其他更复杂的表达式
  * 操作符: 对数据操作，
    * 可以是算术操作符（如 + 或 *）、关系操作符（如 == 或 <）、逻辑操作符（如 && 或 ||），赋值操作符（=、+=）、位操作符、条件操作符、逗号操作符、指针相关操作符、类型相关操作符……
    * 表达式结果：数值，bool，对象
    * 几元运算符：运算符需要几个操作数，3元如：(condition) ? a : b
    * 有优先级和结合性
* 字面值常量：存放在寄存器，有不可寻地址，有值，没有标识符，如1、true、'a'……一望而知值的
* 字面值类型：算数类型、引用和指针等
* 常量表达式:
