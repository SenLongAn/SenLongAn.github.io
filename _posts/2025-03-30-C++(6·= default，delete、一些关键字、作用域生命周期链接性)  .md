...
* 使用等号进行的初始化，或者未显示使用等号 实参->形参，函数返回值 -> 隐式匿名
* 内置类型直接使用参数初始化
* 类对象调用拷贝构造函数或移动拷贝构造函数
...
* 可被聚合初始化条件：它们所有非静态数据成员都是公有的，没有自定义构造函数，没有基类，没有虚函数
...
* 使用有参花括号进行的初始化，
* 它有严格的类型检查，检查失败不会通过编译
* 对于类类型可以调用构造/拷贝构造函数，当列表为空调用默认构造
