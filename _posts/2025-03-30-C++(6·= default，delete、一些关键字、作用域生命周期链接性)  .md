...
* 类：在类(class/struct)内定义的变量/函数，类内可以直接访问，类外通过 类对象/指针/引用/作用域解析运算符 :: 访问（具体是否可以被类外访问取决于成员访问修饰符）
* 局部：在函数/代码块({})中定义，仅在内部访问,且会隐藏外部的同名变量

**生命周期**（3类）：在内存中存在的时间，即创建到销毁
...
**定义**：分配内存空间，并为其分配初始值，作用域与关键字共同决定作用域、生命周期、链接性
...
* 全局作用域
  * 内置类型初始化为0、
  * 指针为nullptr，
  * 对象调用默认构造函数，如果没有默认构造，成员值初始为0
* 局部作用域
  * 内置类型值未定义、
  * 指针未定义，
  * 类对象调用默认构造函数，如果没有默认构造，成员值是未定义的，
* 如果访问和使用未定义的对象，程序将发生错误
