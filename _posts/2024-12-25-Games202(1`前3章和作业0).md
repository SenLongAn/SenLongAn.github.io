...
[查阅WebglAPI和基础教程](https://developer.mozilla.org/zh-CN/docs/Web/API/WebGL_API)
...
* pointlight点光源：点光源的属性，网格默认是立方体，材质是EmissiveMaterial

//

* loadsmodel：
  * 接受一个mtl和obj的文件，
  * 就像点光源需要创建mesh，material，还要额外创建texture并添加到材质
  * 创建meshrender
  * 并添加到渲染器内，
* loadshader：通过THREE.FileLoader()

//

* material材质基类：
  * 构造函数包括shader相关的数据和属性，以及使用的着色器
  * compile编译函数，返回链接的着色器
* lights：EmissiveMaterial材质继承自 Material：包含光的属性，使用了光的着色器，
...
* mesh网格：（保存顶点数据）
  * 构造函数：保存顶点数据
  * cube顶点数据
* meshrender网格渲染器：（这里存储所有渲染需要的：顶点数据，材质属性，着色器……）
  * 构造函数中初始化和创建4个缓冲区用来存储顶点数据，
  * if检查如果有某类数据，bind一遍传入数据再解绑
  * 设置材质属性，编译着色器
  * draw函数：
    * 创建MVP矩阵
    * 设置VAO
...
* WebGLRenderer主渲染器：
  * obj数组，
  * 添加函数:添加到数组，对于light是在这里为它添加meshrender组件
  * render渲染函数:
    * 设置webgl指令
    * for循环所有的物体，启用shaderprogram，传入uniform，调用meshrender.draw渲染
...
* index.html前端：
  ...
  * link链接资源文件
  * 指定src源文件
  * 创建canvas画布<canvas id=

如果渲染不出人物，需要加入,指明使用的资源:
