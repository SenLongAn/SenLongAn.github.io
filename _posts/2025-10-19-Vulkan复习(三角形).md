...
* 图像布局：像素在内存中的组织方式，最适合该操作使用的布局，将获得最佳性能（均以VK_IMAGE_LAYOUT_开头）
  ...
  * VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL：最适合作为附件，用于在fs写入颜色
...
* 流程
  * 初始化：
    * VkAttachmentDescription附件描述，包含附件所使用的图像视图的格式（应该与交换链图像的格式匹配），图像的样本数（抗锯齿），loadOp 和 storeOp 确定在渲染之前和渲染之后如何处理附件中的数据（颜色和深度数据）（LOAD保留附件的现有内容，CLEAR将值清除为常量（清楚色/背景色），DONT_CARE内容未定义）（STORE渲染的内容将存储在内存中以便读取（希望看到结果），DONT_CARE渲染后内容未定义），tencilLoadOp / stencilStoreOp 应用于模板数据，initialLayout、finalLayout图像在渲染通道开始之前、完成后 数据布局（图像布局）
