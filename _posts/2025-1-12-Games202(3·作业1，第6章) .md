...
* webglrender:
  ...
  * render中，变为了两次pass，首先渲染阴影贴图，然后渲染mesh
* 平行光：
  * 包括mesh，材质相关属性，
  * CalcLightMVP: 返回光源的matrix（光空间）
  * 包括FBO的创建
* shadow材质继承主材质：
  * 包含FBO的帧缓冲区，它将在draw时绑定，从而渲染到fbo上，注意在mesh的材质中fbo为空，因此绑定到默认帧缓冲，渲染到屏幕
  * buildPhongMaterial函数: 返回new的新材质，接受各种材质属性和使用的shader（是在loadobj中调用的）
* FBO.js，这个类的构造首先创建帧缓冲和设置属性，创建纹理和设置属性，纹理和缓冲关联，以及错误检查，解绑
* shadow glsl：
  * gl_FragCoord是内置的视口变换后的坐标:其大小范围是x：[0, ScreenWidth]，y：[0, ScreenHeight]，z：[0, 1]
...
* 在mesh的vs着色器，将world pos（经过model）* LightMVP矩阵变换到光空间，获得NDC[-1,1]标准化设备坐标
...
* STEP 3: filtering，调用pcf函数即可，使用penumbra过滤系数，对pcf的过滤范围缩放
...
* 在engine
  * setTransform函数中添加模型变换_旋转部分，这是修改顶点数据
  * Add mesh中设置模型旋转初始值
* Mesh.js中也加入旋转，以便可以接受模型的旋转
* MeshRender.js中也加入旋转，要让相机接受模型的旋转数据
* //灯MVP ？？？
* DirectionalLight.js
...
但阴影没有变，是因为我们没有向shadow glsl中传入旋转后的模型，也就是要实时设置lightMVP
