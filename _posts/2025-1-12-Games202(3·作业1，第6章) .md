...
* 勒让德多项式
  ...
  * 构造勒让德基函数
    ...
    * 根据公式1首先求Pm^m
    * 根据公式2求Pm^m+1
    * 根据公式3，滚动数组循环求解（pmm,pmm+1,pmm+2,pmm+3……pml），它依赖 l-1 和 l-2
  * 计算勒让德系数……略（这里暂时只关心基函数，没有计算系数，因为在sh中只使用它的基函数）
...
* 基函数可视化：
  ...
  * 当3维SH函数被可视化成几何体时,向每个球坐标采样获得结果值，几何体的形状和灰度由结果值的绝对值确定，表面的颜色由结果值的正负性确定，绿色为正值，红色为负值，
* 投影：
...
* 标准示例：
  ...
  * 假设渲染方程有L和V项两项乘积积分
  * 普通复杂度为N即采样次数
  * 将两函数分别展开，用近似函数表示，两项乘积积分，即每个对应的函数（系数*基函数）相乘，再求和，由于正交性，基函数i\==j乘积\==1，则可以化简掉，只保留系数乘积，复杂度为M即函数个数个
* SH函数的笛卡尔版本
