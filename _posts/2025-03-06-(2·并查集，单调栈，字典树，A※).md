...
* 尝试用其他解决方案：
  * 深度优先搜索：从A开始深度搜索，当超过边界/被访问过/到障碍物/找到B，回溯（自底向上），对所有可行方案取最小，在时间上，要遍历所有路径，这样非常耗时，如果要保存路径节点结果，还要大量的存储空间
  * 广度优先搜索：从A开始，不断向外扩展，当某次遇到B时，停止搜索，时间上最坏情况要遍历所有节点，空间上要大量存储
* 对于已知终点位置来说，没必要遍历所有节点和记录所有情况
* Dijkstra：和bfs相似也是向外扩展，但对于每个节点的4个子节点代价不同，代价为距离起点的移动距离，并非像bfs使用queue，而是改用priority_queue，先从代价最小的节点开始扩展
